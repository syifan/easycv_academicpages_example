...
## What's here
- `_pages/cv.md`: EasyCV-powered CV page that loads EasyCV v0.3.2 from the CDN.
...
- Our `_pages/cv.md` keeps a small HTML stub (`#cv-root`) and loads `easycv@0.3.2` and `js-yaml@4.1.0` from the CDN, plus a local `customize_easycv.css` with sample overrides.
